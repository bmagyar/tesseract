...
KDL::JntArray KDLJointKin::getKDLJntArray(const EnvState& state, const std::vector<std::string>& joint_names,
...
bool KDLJointKin::calcFwdKinHelper(Eigen::Isometry3d& pose, const Eigen::Isometry3d& change_base,
                                   const KDL::JntArray& kdl_joints, const std::string& link_name) const
{
    KDL::Frame kdl_pose;
...
bool KDLJointKin::calcFwdKin(Eigen::Isometry3d& /*pose*/, const Eigen::Isometry3d& /*change_base*/,
                             const Eigen::Ref<const Eigen::VectorXd>& joint_angles) const
{
...
bool KDLJointKin::calcFwdKin(Eigen::Isometry3d& pose, const Eigen::Isometry3d& change_base,
                             const Eigen::Ref<const Eigen::VectorXd>& joint_angles, const std::string& link_name,
...
bool KDLJointKin::calcJacobianHelper(KDL::Jacobian& jacobian, const Eigen::Isometry3d& change_base,
                                     const KDL::JntArray& kdl_joints, const std::string& link_name) const
{
...
bool KDLJointKin::calcJacobian(Eigen::Ref<Eigen::MatrixXd> /*jacobian*/, const Eigen::Isometry3d& /*change_base*/,
                               const Eigen::Ref<const Eigen::VectorXd>& joint_angles) const
{
    assert(checkInitialized());
    assert(checkJoints(joint_angles));
...
bool KDLJointKin::calcJacobian(Eigen::Ref<Eigen::MatrixXd> jacobian, const Eigen::Isometry3d& change_base,
                               const Eigen::Ref<const Eigen::VectorXd>& joint_angles, const std::string& link_name,
                               const EnvState& state, const Eigen::Ref<const Eigen::Vector3d>& link_point) const
{
    assert(checkInitialized());
    assert(checkJoints(joint_angles));
    assert(std::find(link_list_.begin(), link_list_.end(), link_name) != link_list_.end());
...
bool KDLJointKin::checkJoints(const Eigen::Ref<const Eigen::VectorXd>& vec) const
{
    ...
    {
        ROS_ERROR("Number of joint angles (%d) don't match robot_model (%d)", static_cast<unsigned>(vec.size()),
                  joint_list_.size());
        return false;
    }
    ...
    for (int i = 0; i < vec.size(); ++i)
    {
        ...
        {
            ROS_WARN("Joint %s is out-of-range (%g < %g < %g)", joint_list_[static_cast<size_t>(i)].c_str(),
                     joint_limits_(i, 0), vec(i), joint_limits_(i, 1));
        }
    }
...
const Eigen::MatrixX2d& KDLJointKin::getLimits() const
{
...
bool KDLJointKin::init(urdf::ModelInterfaceConstSharedPtr model, const std::vector<std::string>& joint_names,
                       const std::string name)
{
    ...
    const int num_active_joints = boost::range::count_if(
        kdl_tree_.getSegments(), [&joint_names](const KDL::SegmentMap::value_type& tree_element) {
    ...
    for (const auto& tree_element : kdl_tree_.getSegments())
    {
        ...
        if ((jnt.getType() != KDL::Joint::None) and containedIn(joint_names, jnt.getName()))
        {
            ...
            std::vector<std::string>::const_iterator link_it =
                std::find(link_list_.begin(), link_list_.end(), seg.getName());
